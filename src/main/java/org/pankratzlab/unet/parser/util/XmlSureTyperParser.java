...
public class XmlSureTyperParser {
  ...
  public static void buildModelFromXML(ValidationModelBuilder builder, Document doc) {
    ...
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_A)
                .text()
                .replaceAll("A", "")
                .split("\\s"))
        .forEach(builder::a);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_B)
                .text()
                .replaceAll("B", "")
                .split("\\s"))
        .forEach(builder::b);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_C)
                .text()
                .replaceAll("Cw", "")
                .split("\\s"))
        .forEach(builder::c);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_DRB1)
                .text()
                .replaceAll("DR", "")
                .split("\\s"))
        .forEach(builder::drb);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_DQB1)
                .text()
                .replaceAll("DQ", "")
                .split("\\s"))
        .forEach(builder::dqb);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_DQA1)
                .text()
                .replaceAll("DQA1\\*", "")
                .split("\\s"))
        .forEach(builder::dqa);
    Arrays.stream(
            labAssignmentSection
                .getElementsByAttributeValue(NAME_TAG, HLA_DPB1)
                .text()
                .replaceAll("DPB1\\*", "")
                .split("\\s"))
        .forEach(builder::dpb);
    builder.bw4(
        labAssignmentSection.getElementsByAttributeValue(NAME_TAG, BW).text().contains("Bw4"));
    builder.bw6(
        labAssignmentSection.getElementsByAttributeValue(NAME_TAG, BW).text().contains("Bw6"));
   ...
   */
  private static void parseHaplotype(
      ValidationModelBuilder builder,
      Element haplotypeXmlSection,
   ...
   */
  private static void parseAlleles(
      String allelesXmlSection,
      ValidationModelBuilder builder,
      Multimap<Strand, HLAType> strandMap,
      String[] loci) {
    ...
    for (int tokenIndex = 2; tokenIndex < tokens.length; tokenIndex++) {
      ...
      if (loci.length > 1
          && token.contains(loci[1])
          && !token.contains(loci[0])
          && strandIndex == 0) {
        strandIndex = 1;
